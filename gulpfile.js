...
gulp.task('styles', function () {
    return gulp.src('src/scss/main.scss')
        ...
        .pipe(gulp.dest('dest/wp-content/themes/trip/css'))
        .pipe($.size({title: 'styles'}));
});

gulp.task('fonts', function() {
    return gulp.src('src/fonts/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/trip/fonts'))
        .pipe($.size({ title: 'fonts' }));
});

gulp.task('img', function() {
    return gulp.src('src/img/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/trip/img'))
        .pipe($.size({ title: 'img' }));
});

gulp.task('layoutImg', function() {
    return gulp.src('src/layoutImg/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/trip/layoutImg'))
...
gulp.task('theme', function() {
    return gulp.src('src/theme/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/trip'))
        .pipe(shell(['mkdir -p dest/wp-content/themes/trip/acf-json']))
        .pipe(shell(['mkdir -p dest/wp-content/themes/trip/../../plugins']))
        .pipe(shell(['mkdir -p dest/wp-content/themes/trip/../../uploads']))
...
gulp.task('wp', function() {
    WP.discover({path: 'dest'}, function( WP ){
        ...
        WP.theme.activate('trip', function( err, results ){
            console.log(err + results);
...
gulp.task('sitemap', function () {
    gulp.src('dest/wp-content/themes/trip/**/*.html', {
            read: false
        })
        .pipe(sitemap({
            siteUrl: 'http://www.e-hamel.com/trip'
        }))
        .pipe(gulp.dest('dest/wp-content/themes/trip'));
});
