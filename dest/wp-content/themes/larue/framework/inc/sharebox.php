...
if(is_sticky() && !is_single()){
	echo '<a href="'.esc_attr(get_the_permalink()).'" class="button viewpost" rel="bookmark">'.esc_html__('View post', 'larue').'</a>';
	$out_class = 'sticky';
} else {
	$out1 = $out2 = $out_class = '';
}
?>
<div class="sharebox <?php echo esc_attr($out_class); ?>">
	<?php
		if(is_sticky() && !is_single()){
			echo '<span class="authorpost">'.esc_html__('By', 'larue').' '.esc_html(get_the_author()).'</span>';
		}
	?>
