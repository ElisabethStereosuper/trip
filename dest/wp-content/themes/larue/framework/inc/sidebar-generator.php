...
class sidebar_generator {
			...
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.onError = function() { alert("<?php esc_html_e('Ajax error. Cannot add sidebar','larue'); ?> ")};
				  	mysack.runAJAX();
					return true;
				}
	...
	public static function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die("alert('".esc_html__('Sidebar already exists, please use a different name.','larue')."')");
		}
		...
		$js = "
			...
			//var textNode = document.createTextNode('[<a href=\'javascript:void(0);\' onclick=\'return remove_sidebar_link($name);\'>".esc_html__('Remove','larue')."</a>]');
			//cellLeft.appendChild(textNode)
	...
	public static function remove_sidebar(){
		...
		$id = sidebar_generator::name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('".esc_html__('Sidebar does not exist.','larue')."')");
		}
	...
	public static function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("<?php esc_html_e('Are you sure you want to remove','larue'); ?> " + name + "<?php esc_html_e('?\nThis will remove any widgets you have assigned to this sidebar.','larue'); ?>");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php esc_html_e('Sidebar Name:','larue'); ?>","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php esc_html_e('Sidebar Generator','larue'); ?></h2>
			
			<p><?php esc_html_e('You can generate as many sidebars as you need. Then you can place them on any page you wish.','larue'); ?></p>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php esc_html_e('Name','larue'); ?></th>
					<th><?php esc_html_e('CSS class','larue'); ?></th>
					<th><?php esc_html_e('Remove','larue'); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo esc_attr($alt); ?>">
					<td><?php echo esc_js($sidebar); ?></td>
					<td><?php echo sidebar_generator::name_to_class($sidebar); ?></td>
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_js($sidebar); ?>',<?php echo esc_html($cnt+1); ?>);" title="<?php esc_html_e('Remove this sidebar','larue'); ?>"><?php esc_html_e('remove','larue'); ?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php esc_html_e('No Sidebars defined','larue'); ?></td>
					</tr>
					<?php
				}
				?>
			...
			<div class="add_sidebar">
				<a href="javascript:void(0);" class="button" onclick="return add_sidebar_link()" title="<?php esc_html_e('Add a sidebar','larue'); ?>"><?php esc_html_e('Create Sidebar','larue'); ?></a>
			</div>
		</div>
		<?php
	}
	...
	*/
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="<?php esc_html_e('Click to toggle','larue'); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e('Sidebars','larue'); ?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p>
						<?php esc_html_e("Select the sidebar you wish to display on this page. If you don't want to display a Sidebar, select the 'Full-Width' Template. Note: You must first create the sidebar under Appearance > Sidebars.",'larue'); ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo esc_attr($i); ?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php esc_html_e('WP Default Sidebar','larue'); ?></option>
							<?php
